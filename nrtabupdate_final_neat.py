...
def update_dashboard(args):
    ...
    if args.get("widget_name", {}):
        jsonvariables = update_widget_info(args["tab_name"], args["widget_name"], args["desired_widget_name"],
                                           jsonvariables, args["dashboard_name"])
...
def update_tab_info(tab_name, desired_tab_name, jsonvariables, dashboard_name):
    pages, new_page, flag, index = find_required_object(jsonvariables, tab_name)
    if not flag:
        print('came to else')
        req_guid = get_req_dashboard_guid(dashboard_name)
        print(req_guid)
...
def main():
    ...
    parser.add_argument("-dt", "--desired_tab_name", help="Desired Tab name")
